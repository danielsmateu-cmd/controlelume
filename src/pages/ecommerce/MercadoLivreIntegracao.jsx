        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          {listings.length === 0 ? (
            <div className="p-12 text-center text-gray-400">
              ...
              <p>Nenhum an�ncio importado.</p>
              <p className="text-sm mt-1">Clique em "Sincronizar" para importar seus an�ncios do ML.</p>
            </div>
          ) : (
...
<tr className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-center">
                        {editingStock === listing.id + '_ml' ? (
                          ...
                          <button onClick={() => { setEditingStock(listing.id + '_ml'); setStockValue(listing.stock_ml ?? ''); }} className="flex items-center gap-1 mx-auto font-bold text-yellow-700 hover:text-yellow-800">
                            {listing.stock_ml ?? '�'} <Edit2 size={10} />
                          </button>
                        )}
                      </td>
                      <td className="px-4 py-3 text-center">
                      ...
                      <td className="px-4 py-3 text-center">
                        {listing.status === 'active' ? (
                          ...
                          <button onClick={() => handleReactivateItem(listing)} title="Reativar an�ncio" className="p-1.5 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors">
                            <Play size={14} />
                          </button>
                        ) : null}
                      </td>
      ...
      {activeTab === 'historico' && (
        <div className="space-y-4">
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center gap-3">
            ...
            <div>
              <label className="text-xs text-gray-500 block mb-1">At�</label>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            {salesHistory.length === 0 ? (
              <div className="p-12 text-center text-gray-400">
                ...
                <p>Selecione um per�odo e clique em Buscar.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
